...
class RTLearner(object):
    ...
    def addEvidence(self, dataX, dataY):
        ...
        # print "Determining random feature i to split on"
        ...
        # print corr_np.shape[0]

        # Get random feature
        random_feat_i = int(np.random.choice(len(corr_coefs), 1, replace=False))

        # print random_feat_i

        # Split the data according to the best feature
        split_val = np.median(dataX[:, random_feat_i])
        # print "Split Value: "
        # print split_val

        # Check if split val is sending the whole tree:
        left_idx = dataX[:, random_feat_i] <= split_val
        ...
        right_tree = self.addEvidence(dataX[dataX[:, random_feat_i] > split_val],
                                      dataY[dataX[:, random_feat_i] > split_val])

        root = np.array(([random_feat_i, split_val, 1, left_tree.shape[0]+1]),)
